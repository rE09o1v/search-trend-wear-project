...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("🚀 全ブランド一括スクレイピング")
    st.info("brands.jsonに登録されているすべてのブランドのデータを一括で取得します。")
    
    col1, col2 = st.columns([1, 2])
    with col1:
        if st.button("全ブランド一括実行", type="secondary", key="btn_bulk_scrape_all"):
            with st.spinner("全ブランドの一括スクレイピングを実行中... この処理には時間がかかります。"):
                try:
                    # プログレスバーとステータス表示のためのプレースホルダー
                    progress_placeholder = st.empty()
                    status_placeholder = st.empty()
                    
                    # 一括スクレイピング実行前の通知
                    status_placeholder.info("一括スクレイピングを開始します...")
                    
                    # main_scrape_all()を実行
                    main_scrape_all()
                    
                    # 成功メッセージ
                    status_placeholder.success("✅ 全ブランドの一括スクレイピングが完了しました！")
                    st.balloons()
                    
                    # キャッシュをクリアして最新データを反映
                    load_price_data_cached.clear()
                    time.sleep(2)  # メッセージを表示するための短い待機
                    st.rerun()
                    
                except Exception as e:
                    st.error(f"❌ 一括スクレイピング中にエラーが発生しました: {e}")
    
    with col2:
        st.markdown("""
        **⚠️ 注意事項:**
        - この処理は非常に時間がかかります（数十分〜数時間）
        - brands.jsonに登録されている全サイト・全ブランドが対象です
        - 実行中はページを閉じないでください
        - サイトへの負荷を考慮し、適切な間隔でアクセスします
        """)
